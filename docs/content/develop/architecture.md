...
Conductor is a deterministic Bun + TypeScript runner. Headless coding agents sit **inside** pipeline stages. Routing, gating, retries, and label transitions are **plain code** — never ask the model something a script can decide (for example, whether tests passed).
...
├── server.ts           # conductor ui: Bun.serve, SSE, poll/run triggers
...
└── log.ts              # transcripts + cost under <target>/.conductor/runs/<issue>/
...
- **Plan-comment marker is exactly `<!-- conductor:plan -->`** (`PLAN_MARKER` in `stages/triage.ts`). Dev and replan find plans by the newest comment containing it.
...
- **Agents never run git/gh side effects.** The runner owns commit, push, PR creation, and labels. Conductor never merges and never touches the base branch.
...
4. Stages emit through `ctx.onEvent`. The CLI prints events; `conductor ui` also broadcasts them over SSE.
...
Release assets are `dist/conductor-*` plus `checksums.txt`. `install.sh` at the repo root is the checksum-verified installer.
